...
# Step 2: Return name and mass for planets with exactly 7-letter names
...
# Step 5: Return name and color for planets with color containing "blue"
df_blue = pd.read_sql("""
    SELECT name, color FROM planets
    WHERE color LIKE '%blue%';
""", conn1)

# Part III: Ordering and Limiting

# Step 6: Return name, age, breed for hungry dogs, sorted youngest to oldest
...
# Step 7: Return name, age, hungry for hungry dogs aged 2-7, sorted alphabetically by name
...
# Step 10: Return total number of homeruns hit by Babe Ruth
df_hr_total = pd.read_sql("""
    SELECT SUM(HR) as total_homeruns FROM babe_ruth_stats;
""", conn3)

# Part V: Grouping and Aggregation

# Step 11: Return team name and number of years for each team
df_teams_years = pd.read_sql("""
    SELECT team, COUNT(*) as number_years FROM babe_ruth_stats
    GROUP BY team;
""", conn3)

# Step 12: Return team name and average at bats for teams averaging over 200
df_at_bats = pd.read_sql("""
    SELECT team, AVG(at_bats) as average_at_bats FROM babe_ruth_stats
    GROUP BY team
